...
shinyServer(
  ...
  output$plot1 <- renderPlot({
    ggplot(inputdata1(),aes(x=DATE_TIME,y=USED_Q))+geom_point(color='pink')+geom_line(color='blue')+scale_x_date(breaks=scales::date_breaks(ifelse(input$d==1,'week',paste(input$d,'weeks',sep = ' '))))
  })
  output$plot2 <- renderPlot({
    ggplot(inputdata2(),aes(x=DATE_TIME,y=LOST_Q/10000))+geom_point(aes(color=Normal))+scale_color_hue('Is LOST_RATE Normal?',breaks=c(0,1),labels=c('No','Yes'))+
      xlab('Date')+ylab('Volume(w)')+scale_x_date(breaks=scales::date_breaks(ifelse(input$d==1,'week',paste(input$d,'weeks',sep = ' '))))+
      geom_line(aes(y=SALE_Q/10000),color='green4')
